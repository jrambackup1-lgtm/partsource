...
export default function CatalogApp() {
  ...
  return <div className="app-shell">
    ...
    <div className="notice" role="note" data-inspector-background><strong>{dataLabel}</strong><span>{view.notice}</span><span className="release" title={view.digest}>Catalog: {view.releaseId}</span></div>
...
function Inspector({ detail, onClose }: { detail: NonNullable<ReturnType<typeof projectCatalogView>['detail']>; onClose: () => void }) {
  ...
  return <div className={`inspector-backdrop ${mobile ? 'is-modal' : 'is-complementary'}`} onMouseDown={event => { if (mobile && event.target === event.currentTarget) onClose(); }}><aside ref={panel} className="inspector" role={mobile ? 'dialog' : 'complementary'} aria-modal={mobile ? true : undefined} aria-labelledby="inspector-title"><header><div><p className="eyebrow">Part inspector</p><h2 id="inspector-title">{id}</h2><p>{detail.familyLabel}</p></div><button ref={close} className="close-button" onClick={onClose} aria-label="Close inspector">×</button></header>
    ...
    <section aria-labelledby="identifier-title"><h3 id="identifier-title">Part number</h3><dl className="identity-list">{mappingProvenance.map(({ identifier, source }) => <div key={identifier.mappingId}><dt>Part number</dt><dd>{identifier.identifier}<small>{sourceLabel(source)}</small></dd></div>)}</dl>
      <details className="technical-evidence"><summary>Technical identifiers</summary><dl className="identity-list">{mappingProvenance.map(({ identifier, source }) => <div key={identifier.mappingId}><dt>{identifier.namespaceLabel}</dt><dd>{identifier.identifier}<small>mapping {identifier.mappingId} · {identifier.provenanceId}</small><small>{sourceLabel(source)} · {source?.sourceId ?? 'Source unavailable'}</small>{source?.evidenceRefs.map(ref => <small key={ref.ref}>{ref.ref}</small>)}</dd></div>)}<div><dt>Internal revision</dt><dd>{detail.configurationRevisionId}</dd></div></dl></details>
    </section>
  </aside></div>;
}
