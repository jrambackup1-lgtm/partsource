...
4. Replace the currently observed public POC deployment only after explicit publication approval. At `2026-08-15T14:00:25Z`, the canonical site still served `PartSource Progressive Catalog POC` and `/partsource/release.json` returned HTTP 404. Recovery deployment is pending as of 2026-08-16. Retain the replacement workflow run, artifact identity, deployment URL, and successful post-deploy exact-byte verification.
