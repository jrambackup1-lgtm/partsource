...
**Status:** f-series fully resolved (2026-08-16, §17): u1–u6 shipped to production and live-verified (f1); the real 27k catalog is the active **dev** product with truthful labels (f2, decision D6); the main UI carries no diagnostic identifiers by default (f3); exact-PN matches land on their page with a visible highlight (f4); user-path browser suites cover both the real and default paths and a deploy-truth guard connects claims to deployed bytes (f5). Production remains on the synthetic catalog per the publication gates (mechanical review, field adjudication, publication approval). Correction recorded by the second audit (`live-product-root-cause-2026-08-16.md`): the earlier "production-verified" deployment (run 31926209080, source `01a214b`) predated u1–u6 and verified only the pre-recovery build. Prior closeout: eight-phase recovery complete.
...
| f2 — real catalog active dev product | **resolved** | Decision D6 (decisions/u1-real-catalog-data-decisions.md): dev-server default real (`import.meta.env.DEV`), `?catalog=synthetic` opt-out, production/preview always synthetic with the `catalog` parameter inert (no fetch, no warning; engine treats app-owned-only URLs as empty state — regression-tested). Notice/pill name the active data; command-free fail-closed error copy; deep links hydrate against the active package (no spurious link warning). Verified: dev default 26,953 parts + 79-family `M4 screw` chooser; prod preview inert; lint/engine/browser suites green (mobile scroll-lock test stabilized, see f5). |
| f3 — main UI de-diagnose | **resolved** | Inspector identity section is now a clean "Part number" (value + one-line source label); mapping/provenance/source/evidence-ref/internal-revision strings moved into a collapsed "Technical identifiers" disclosure (optional, audit-reachable). Notice bar drops the digest text (tooltip only), keeps "Catalog: {releaseId}". Browser spec asserts the disclosure-state contract; real-catalog probe verified (`92655A687` + "Cofounder dataset (local dev)", disclosure closed). |
| f4 — exact-match visibility at real scale + URL truth | **resolved** | Exact-match page-follow: resolution/sort transitions land on the page containing the highlighted row (manual paging never snaps back); scroll-into-view re-fires after the page lands. urlWarning computed against the active package (with f2). Chooser symmetry in `chooserResolution`: one remaining family opens directly (engine regression added). Real-catalog probe: `92655A331` → page 2 of 3, highlighted row in viewport, truthful banner + URL. Suites green (engine, url-history, real-package, browser 16/16). |
| f5 — user-path test integrity | **resolved** | Real-path spec `catalog-real-path.spec.ts` via `playwright.dev.config.ts` (`npm run test:browser:real` — dev server, serialized workers): 5/5 green in 2.2 m, covering dev default-real labels, owner phrasing `M4 screw` chooser with kept constraint, exact-PN correct family + in-viewport highlighted row, PN-titled rows/detail with optional diagnostics, absent-PN echo failure. Preview suite pins the synthetic default path (singular `M4 screw`; PN-shaped input → honest class) — 17/17. Deploy-truth guard `npm run deploy:check` demonstrated biting against stale production. CI deploy workflow runs the real path after the preview browser step. Mobile scroll-lock test stabilized (with f2). Evidence-citation rule recorded below. |

**Evidence-citation rule (f5, process):** a ticket resolution may cite only verifications that are recorded in the file it references. Unrecorded verifications ("live dev-server Playwright verification", ad-hoc counts) are not evidence — this rule exists because u1/u2 cited two such verifications that did not exist in the referenced evidence file (second audit, RC5).
