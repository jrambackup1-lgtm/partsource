...
import {
  ...
  findCatalogPart,
  fuse,
  isMcMasterIdentifier,
  UNSUPPORTED_INPUT_COPY,
  normalizeMcMasterIdentifier,
  resolvePartIdentity,
  MCMASTER_CROSSES,
  type Part,
...
test('catalog: every entry has a valid partNumber and required fields', () => {
  for (const p of db) {
    ...
    assertEqual(p.mcmasterPrice, 0, `catalog entry must not expose a price: ${p.partNumber}`);
...
test('crosses: no McMaster cross maps publish before source clearance', () => {
  assertEqual(MCMASTER_CROSSES.length, 0, 'no public cross-reference mappings');
...
test('findCatalogPart: verified crosses resolve; prototype and unknown MPNs do not resolve locally', () => {
  assertEqual(findCatalogPart('91290A115'), null, 'uncleared MPN cross does not resolve to own PN');
...
test('prototype socket record is not bundled into local fallback', () => {
  ...
  assertEqual(result.state, 'unsupported-input', 'prototype MPN is not a local exact lookup');
  ...
  assertEqual(result.part.appNote, UNSUPPORTED_INPUT_COPY, 'unsupported copy');
});
...
test('lookup result states: match, search, unsupported, and invalid are separate', () => {
  assertEqual(normalizeMcMasterIdentifier(' 91290a115 '), '91290A115', 'McMaster normalization');
  assertEqual(isMcMasterIdentifier('91290A115'), true, 'valid McMaster identifier');
  assertEqual(resolvePartIdentity('din912-m4x12').state, 'configuration-match', 'own PN is a local configuration match');
  assertEqual(resolvePartIdentity('91290A115').state, 'unsupported-input', 'uncleared cross is unsupported');
  assertEqual(resolvePartIdentity('DIN912-M4X12 A2').state, 'configuration-search', 'clear catalog search is not a supplier claim');
  assertEqual(resolvePartIdentity('DIN912-M4X13').state, 'configuration-search', 'ambiguous unavailable size is not mapped to a neighboring SKU');
  assertEqual(resolvePartIdentity('M5 socket head cap screw x 16mm').state, 'configuration-search', 'usable specs are configuration searches');
  assertEqual(resolvePartIdentity('99136A101').state, 'unsupported-input', 'unrecognized MPN is unsupported');
  assertEqual(resolvePartIdentity('   ').state, 'invalid-input', 'blank input is invalid');
});

test('exact McMaster lookup never fuzzy-promotes suggestions', () => {
  const nearMiss = resolvePartIdentity('91290A116');
  assertEqual(nearMiss.state, 'unsupported-input', 'near McMaster miss stays unsupported');
  assertEqual(nearMiss.part.partNumber, '91290A116', 'no neighboring verified part substituted');
  assertEqual(nearMiss.part.appNote, UNSUPPORTED_INPUT_COPY, 'public unsupported copy');
  assertEqual(findCatalogPart('91290A116'), null, 'near McMaster miss does not resolve locally');
});
...
test('parseCustomPart: decoded spec has no price estimate', () => {
  const p = parseCustomPart('M5 socket head cap screw x 16mm');
  assertEqual(p.unindexed, true, 'guessed parts are always unindexed');
  assertEqual(p.mcmasterPrice, 0, 'decoded spec must not carry an estimate');
});
