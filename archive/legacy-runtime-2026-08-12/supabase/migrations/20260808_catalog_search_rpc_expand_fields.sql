...
as $$
  ...
  where not exists (select 1 from exact_reference)
    ...
    and (
      title ilike '%' || p_query || '%'
      or thread ilike '%' || p_query || '%'
      or pitch ilike '%' || p_query || '%'
      or length ilike '%' || p_query || '%'
      or material ilike '%' || p_query || '%'
      or finish ilike '%' || p_query || '%'
      or drive ilike '%' || p_query || '%'
      or standard ilike '%' || p_query || '%'
    )
  limit 25;
$$;
